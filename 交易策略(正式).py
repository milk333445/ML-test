...
for i in range(1,246):
    if data['prediction'][i]==data['label'][i+1]==1 and data['sentiment'][i-1] > data['ma'][i-1]:
        AP=AP*(1+abs(data['fluctuation'][i+1]))*(1-0.001425)
        list.append(AP - 1)
    elif data['prediction'][i]==data['label'][i+1]==0 and data['sentiment'][i-1] < data['ma'][i-1]:
        AP=AP*(1+abs(data['fluctuation'][i+1]))*(1-0.001425)
        list.append(AP - 1)
    elif data['prediction'][i]==1 and data['label'][i+1]==0 and data['sentiment'][i-1] > data['ma'][i-1]:
        AP = AP * (1 - abs(data['fluctuation'][i + 1]))*(1-0.001425)
        list.append(AP - 1)
    elif data['prediction'][i]==0 and data['label'][i+1]==1 and data['sentiment'][i-1] < data['ma'][i-1]:
        AP = AP * (1 - abs(data['fluctuation'][i + 1]))*(1-0.001425)
        list.append(AP - 1)
    else:
        AP=AP
...
percentage_r = []
for i in range(1,246):
    if data['prediction'][i]==data['label'][i+1]:
        AP=AP*(1+abs(data['fluctuation'][i+1]))*(1-0.001425)
        list.append(AP - 1)
    else:
        AP=AP*(1-abs(data['fluctuation'][i+1]))*(1-0.001425)
